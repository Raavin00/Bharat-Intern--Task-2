var taskObj = {
  ...
  changeTaskStatus: function(self) {
      ...
      if (self.value == "delete" || self.value == "start" || self.value == "stop") {
...
window.addEventListener("load", function() {
  ...
  setInterval(function() {
      ...
      for (var i = 0; i < dataStarted.length; i++) {
          var dataStartedObj = JSON.parse(dataStarted[i].getAttribute("data-started"));
              ...
              if (projects[a].id == dataStartedObj.project) {
                  for (var b = 0; b < projects[a].tasks.length; b++) {
                      if (projects[a].tasks[b].id == dataStartedObj.task) {
                          var logs = projects[a].tasks[b].logs;
                          startTime = logs[logs.length - 1].startTime;
                          break;
                      }
                  }
                  break;
